...
io.sockets.on('connection', function(socket) {
  ...
  socket.on('new user', function(data) {
    ...
    if (nicknameTaken) {
      ...
      socket.set("nickname", data.nickname, function() {
        ...
        // Envoyer un message au bot Telegram pour informer qu'un nouveau participant a rejoint le chat
            bot.sendMessage(process.env.TELEGRAM_CHAT_ID, `${data.nickname} a rejoint le chat !`);
      });
  ...
  socket.on('outgoing', function(data) {
    ...
    socket.get('nickname', function(err, nickname) {
      ...
      // Envoyer un message au bot Telegram pour informer qu'un nouveau message a été envoyé
        bot.sendMessage(process.env.TELEGRAM_CHAT_ID, `${nickname}: ${data.message}`);
    });
  });
